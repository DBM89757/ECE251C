...
function [pyr,pind] = buildWpyr(im, ht, filt)
if (nargin < 1)
...
if (ht <= 0)
  ...
  if (im_sz(2) == 1)
    [c,l] = wavedec(im(:),1,filt);
    lolo = appcoef(c,l,filt);
    hihi = detcoef(c,l,1);
  elseif (im_sz(1) == 1)
    [c,l] = wavedec(im(:),1,filt);
    lolo = appcoef(c,l,filt);
    ...
    [c,s]=wavedec2(im,1,filt);
    [lohi,hilo,hihi] = detcoef2('all',c,s,1);
    lolo = appcoef2(c,s,filt,1); 
...
%     hihi = corrDn(hi, hfilt', edges, [1 2], [1 2]); % diagonal
  ...
  [npyr,nind] = buildWpyr(lolo, ht-1, filt);
